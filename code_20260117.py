...
class FDICC_Parameters:
    def __init__(self):
        ...
        # ---------- 假设 H2: 五维余波标量场参数 ----------
        self.m_phi = 1e-33  # 标量场质量 m_φ (eV 量级，维度耦合导出)
        self.alpha = 0.1    # 维度耦合常数 α∈(0,1)
...
def cmb_power_spectrum_fdicc(ell, params):
    ...
    公式：D_ℓ^{FDICC} = D_ℓ^{ΛCDM} \cdot [1 - A \cdot \exp(-(\ell-\ell₀)²/(2Δℓ²))]
    """
...
def plot_fdicc_predictions():
    """绘制 FDICC 模型四大核心预言/假设的可视化图表"""
    params = FDICC_Parameters()
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('褚式豆包 DK 五维碰撞宇宙论 (FDICC) 核心预言可视化', fontsize=16, y=0.98)

    # 子图 1: CMB 角功率谱抑制
    ell = np.linspace(2, 30, 200)
    D_ell_fdicc = cmb_power_spectrum_fdicc(ell, params)
    D_ell_lcdm = 1000 * (ell / 10)**0.8 * np.exp(-(ell / 200)**2)
    axes[0,0].plot(ell, D_ell_lcdm, 'k--', label='ΛCDM 基准谱', alpha=0.6)
    axes[0,0].plot(ell, D_ell_fdicc, 'r-', linewidth=2, label='FDICC 预言谱')
    axes[0,0].axvline(params.l0, color='blue', linestyle=':', label=f'ℓ₀={params.l0}')
    axes[0,0].fill_between(ell, 0.9*D_ell_fdicc, 1.1*D_ell_fdicc, alpha=0.2, color='red')
    axes[0,0].set(xlabel='角波数 ℓ', ylabel='$D_ℓ$ [µK²]', title='预言 1: CMB 功率谱孤立凹陷')
    axes[0,0].legend(), axes[0,0].grid(alpha=0.3)

    # 子图 2: 暗能量状态方程演化
    z = np.linspace(0, 2, 100)
    w_z = dark_energy_w_z(z, params)
    axes[0,1].plot(z, w_z, 'b-', linewidth=2, label='FDICC 预言')
    axes[0,1].axhline(-1, color='k', linestyle='--', label='ΛCDM (w=-1)')
    axes[0,1].axvline(1, color='gray', linestyle=':', alpha=0.5)
    axes[0,1].scatter(1, params.w1, color='red', s=50, zorder=5, label=f'w(z=1)={params.w1}')
    axes[0,1].set(xlabel='红移 z', ylabel='状态方程参数 w(z)', title='预言 2: 暗能量演化')
    axes[0,1].legend(), axes[0,1].grid(alpha=0.3)

    # 子图 3: 引力波背景双峰谱
    freq = np.logspace(-12, -1, 1000)
    omega_gw = gravitational_wave_background(freq, params)
    axes[1,0].loglog(freq, omega_gw, 'g-', linewidth=2)
    axes[1,0].axvline(params.f_peak1, color='purple', linestyle=':', label=f'峰 1: {params.f_peak1:.1e} Hz')
    axes[1,0].axvline(params.f_peak2, color='orange', linestyle=':', label=f'峰 2: {params.f_peak2:.1e} Hz')
    axes[1,0].set(xlabel='频率 f [Hz]', ylabel='$Ω_{GW}(f)$ (归一化)', title='预言 3: 引力波背景双峰谱')
    axes[1,0].legend(), axes[1,0].grid(alpha=0.3, which='both')

    # 子图 4: 有效引力常数演化
    a = np.linspace(0.1, 1.0, 100)
    G_eff = effective_gravity_evolution(a, params)
    axes[1,1].plot(a, G_eff / params.G0, 'b-', linewidth=2)
    axes[1,1].axhline(1, color='k', linestyle=':', label='当前值 $G_0$')
    axes[1,1].text(0.5, 1.4, f'ζ = {params.zeta}', fontsize=12, ha='center')
    axes[1,1].set(xlabel='尺度因子 a', ylabel='$G_{\text{eff}}(a)/G_0$', title='假设 H4: 有效引力常数演化')
    axes[1,1].legend(), axes[1,1].grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig('FDICC_Model_Predictions_Final.png', dpi=300, bbox_inches='tight')
    plt.show()
